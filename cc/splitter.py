...
class FrameProcessor:
    def __init__(self):
        ...
        self.gray_region = np.zeros((60, 60), dtype=np.uint8)
        ...
        self.tracking_region = np.zeros((60, 60, 3), dtype=np.uint8)  # Doubled from 30x30 to 60x60
    ...
    def get_tracking_region(self, frame, cx, cy):
        """Get a 60x60 region around the given center point"""
        # Calculate region bounds with larger area
        x1 = max(0, cx - 30)  # Increased from 15 to 30
        y1 = max(0, cy - 30)  # Increased from 15 to 30
        x2 = min(frame.shape[1], cx + 30)
        y2 = min(frame.shape[0], cy + 30)
